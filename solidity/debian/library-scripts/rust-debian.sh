...
get_common_setting() {
    if [ ! -f "/tmp/library-scripts/vsdc-settings.env" ]; then
        curl -sfL "https://aka.ms/vscode-dev-containers/script-library/settings.env" 2>/dev/null -o /tmp/library-scripts/vsdc-settings.env || echo "Could not download settings file. Skipping."
    fi
    if [ -f "/tmp/library-scripts/vsdc-settings.env" ]; then
        local multi_line=""
        if [ "$2" = "true" ]; then multi_line="-z"; fi
        local result="$(grep ${multi_line} -oP "$1=\"?\K[^\"]+" /tmp/library-scripts/vsdc-settings.env | tr -d '\0')"
